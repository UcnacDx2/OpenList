...
func parseHjqDeviceProfile(raw string) (hjqDeviceProfile, error) {
	...
	if strings.TrimSpace(raw) == "" {
		return profile, errors.New("device_profile is empty")
	}
	if err := utils.Json.UnmarshalFromString(raw, &profile); err != nil {
		return profile, fmt.Errorf("failed to parse device_profile: %w", err)
	}
	...
	for name, value := range required {
		if strings.TrimSpace(value) == "" {
			return profile, fmt.Errorf("device_profile missing %s", name)
		}
	}
	return profile, nil
}
...
func (d *Yun139) loginWithHjqPassword() (string, error) {
	...
	device, err := parseHjqDeviceProfile(d.DeviceProfile)
	if err != nil {
		return "", err
	}
